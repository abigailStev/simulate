...
if __name__ == "__main__":
	...
	freq = 401.0
	bb_spec = "spectra/100000s_mean_nopoiss.fak"
	pl_spec = "spectra/100000s_mean_nopoiss.fak"
	amp_ci = 0.065
	amp_ref = 0.065
	phase_spec = 0.0
	num_seconds = 4
	...
	############################
	for num_segments in xrange(1, 201): # 'i' tracks the number of segments
...
# 	mean_ps_ref /= num_segments
# 	mean_rate_ref /= num_segments
	mean_curve_ci /= num_segments
	mean_curve_ref /= num_segments
...
# 	ccf_filtered, ccf_error = crosscorr.cs_to_ccf_w_err(cs_avg, dt, n_bins, num_seconds, num_segments, mean_rate_whole_ci, mean_rate_whole_ref, mean_power_ci, mean_power_ref)
	
	
	
# 	sim_lc.plot_curves(n_bins, mean_curve_ci[:,6], mean_curve_ref, "plot.png")

# 	power_spectra_things(mean_ps_ref, dt, n_bins, args.num_seconds, num_segments, mean_rate_ref)\
